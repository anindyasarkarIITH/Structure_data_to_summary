...
class BeamSearchDecoder(object):
  ...
  def __init__(self, model, batcher, vocab):
    ...
    self._vocab = vocab
    print ("^^^^^^^^^^")
    print (self._vocab._count)
    self._saver = tf.train.Saver() # we use this to load checkpoints for decoding
    self._sess = tf.Session(config=util.get_config())
    #self._sess = tf.Session()
    # Load an initial checkpoint to use for decoding
    ckpt_path = util.load_ckpt(self._saver, self._sess)
    #saver = tf.train.import_meta_graph('/home/anindya/Documents/structure_data_to_summary/myexperiment/train/model.ckpt-95.meta')
    #saver.restore(self._sess,tf.train.latest_checkpoint('/home/anindya/Documents/structure_data_to_summary/myexperiment/train/'))
    print ("#####")
