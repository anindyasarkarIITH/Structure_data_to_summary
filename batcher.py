...
class Example(object):
  ...
  def pad_encoder_input(self, max_len, pad_id):
    ...
    while len(self.enc_input_content) < max_len:
      self.enc_input_content.append(pad_id)
      self.enc_input_field.append(pad_id)
...
class Batch(object):
  ...
  def __init__(self, example_list, hps, vocab):
    ...
    Args:
       ...
       vocab: Vocabulary object
    """
    self.pad_id = vocab.word2id(data.PAD_TOKEN,field=False) # id of the PAD token used to pad sequences
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    max_enc_seq_len = max([ex.enc_len_content for ex in example_list])
    ...
    self.enc_batch_field = np.zeros((hps.batch_size, max_enc_seq_len), dtype=np.int32)
    self.enc_batch_content = np.zeros((hps.batch_size, max_enc_seq_len), dtype=np.int32)
    self.enc_lens = np.zeros((hps.batch_size), dtype=np.int32)
    self.enc_padding_mask = np.zeros((hps.batch_size, max_enc_seq_len), dtype=np.float32)

    # Fill in the numpy arrays
    for i, ex in enumerate(example_list):
      self.enc_batch_field[i, :] = ex.enc_input_field[:]
      self.enc_batch_content[i, :] = ex.enc_input_content[:]
      self.enc_lens[i] = ex.enc_len_content
      for j in xrange(ex.enc_len_content):
  ...
  def store_orig_strings(self, example_list):
    ...
    self.original_contents = [ex.original_content for ex in example_list] # list of lists
    self.original_fields = [ex.original_field for ex in example_list] # list of lists
    self.original_summaries = [ex.original_summary for ex in example_list] # list of lists
    self.original_summaries_sents = [ex.original_summary_sents for ex in example_list] # list of list of lists
...
class Batcher(object):
  ...
  def __init__(self, data_path, vocab, hps, single_pass):
    ...
    Args:
      data_path: tf.Example filepattern.It includes three features summary_example,content_example,field_example
      vocab: Vocabulary object
      hps: hyperparameters
    ...
    if single_pass:
      ...
      self._num_example_q_threads = 2 # num threads to fill example queue
      self._num_batch_q_threads = 2  # num threads to fill batch queue
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (summary_text, content_text, field_text) = input_gen.next() # read the next example from file. article and abstract are both strings.
      ...
      summary_sentences = [sent.strip() for sent in data.abstract2sents(summary_text)] # Use the <s> and </s> tags in abstract to get a list of sentences.
      
      example = Example(content_text, field_text, summary_sentences, self._vocab, self._hps) # Process into an Example.
  ...
  def fill_batch_queue(self):
    ...
    while True:
      if self._hps.mode != 'decode':
        ...
        inputs = sorted(inputs, key=lambda inp: inp.enc_len_content) # sort by length of encoder sequence
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      try:
        summary_text = e.features.feature['summary_example'].bytes_list.value[0] # the article text was saved under the key 'article' in the data files
        content_text = e.features.feature['content_example'].bytes_list.value[0] # the abstract text was saved under the key 'abstract' in the data files
        field_text = e.features.feature['field_example'].bytes_list.value[0] # the abstract text was saved under the key 'abstract' in the data files
      except ValueError:
        tf.logging.error('Failed to get article or abstract from example')
        continue
      if len(summary_text)==0: # See https://github.com/abisee/pointer-generator/issues/1
        tf.logging.warning('Found an example with empty article text. Skipping it.')
      else:
        yield (summary_text, content_text, field_text)
